...
#include "ns3/callback.h"
//#include "scratch/meshSyncTest.h"
...
public:
  ...
  void SetNodeContainer(NodeContainer node);
   ...
   */
  uint64_t GetSimTimePast();
  void SetSimTimePast(uint64_t simTimePast);
  void UpdateNodeClock();
...
private:
  ...
  int RetrieveIDFromMAC(WifiMacHeader const *hdr);
  void Synchronize(int nodeID, uint64_t timeStamp);
  void CalculateOffset(uint64_t receivedNodeClock, uint64_t beaconTimeStamp, int nodeID);
  int ConvAddressToInt(Mac48Address address);
  void Output();
  bool IsSynchronized();
  bool FirstBeacon();
...
private:
  ...
  std::map <int, int> TSF_list;
  std::map <int, int> BEACON_LIST;
