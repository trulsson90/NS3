...
// Method which creates node woth nodeclock and adds to a node cointainer
void
NodeContainer::Create (uint32_t n, uint64_t nodeClock)
{
  for (uint32_t i = 0; i < n; i++)
    {
      m_nodes.push_back (CreateObject<Node> (nodeClock));
    }
}
