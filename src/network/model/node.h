...
public:
  ...
  uint64_t GetNodeTime (void);

  std::map <int, int> GetMapNode(void);

  void SetMapNode(int nodeID, int tOffset);

  void SetNodeTime(uint64_t nodeTimer);

  int ReceivedBeacons();

  void SetReceivedBeacons(int recBeacons);

  int GetOffsetFromNodes(int node1, int node2);
...
private:
  ...
  long 	  m_clock;		//Test Time for node
