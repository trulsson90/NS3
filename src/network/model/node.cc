...
Node::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Node")
    ...
    .AddAttribute ("SystemId", "The systemId of this node: a unique integer used for parallel simulations.",
                   ...
                   MakeUintegerChecker<uint32_t> ())
	.AddAttribute ("Timer", "The timer for the node",// Implemented new attribute for node
			       TypeId::ATTR_GET || TypeId::ATTR_SET,
				   UintegerValue(0),
				   MakeUintegerAccessor (&Node::m_clock),
...
Node::Node(uint64_t nodeTime)
  : m_id (0),
	m_clock(nodeTime)
{
  NS_LOG_FUNCTION (this << m_clock);
  Construct ();
}
...
std::map<int, int>
Node::GetMapNode(void)
{
	return TSF_list;
}

void
Node::SetMapNode(int nodeID, int tOffset)//create and insert into map
{
	TSF_list[nodeID] = tOffset;
}

int
Node::ReceivedBeacons()
{
	return receivedBeacons;
}

void
Node::SetReceivedBeacons(int recBeacons)
{
	receivedBeacons = recBeacons;
}
int
Node::GetOffsetFromNodes(int node1, int node2)
{
	int value;
	try {
	value = nodeL.GetNode(node1)->GetMapNode().at(node2);
	}
	catch (...) {
		return 0;
	}
...
void
Node::SetNodeTime(uint64_t nodeTime)
{
	m_clock = nodeTime;
}
